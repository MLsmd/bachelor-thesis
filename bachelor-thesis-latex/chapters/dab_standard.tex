...
\node [block, text width=18em](ensemble){\dq Campus Radio\dq };
\node [block, below=0.5cm of ensemble.190, anchor=north, text width=7em](sender1){\dq CELs Angles\dq}; 
\node [block, below=0.5cm of ensemble.350, text width=7em](sender2){\dq KIT Tune\dq};
\node [block, text width=5em, below=1.5cm of $(sender2.west)!0.5!(sender1.east)$](traffic){\dq Stauinfo\dq };
\node [block, left=2mm of traffic](news){\dq Nachrichten\dq };
\node [block, text width=5em, left=2mm of news](audio1){\dq Audio 1\dq };
\node [block, right=2mm of traffic](audio2){\dq Audio 2\dq };
...
\todo{Struktur mit Ensemble Services als Grafik}
...
xlabel={Frequenz},
ylabel={$\frac{sin(\frac{f \pi}{\Delta f})}{\frac{f \pi}{\Delta f}}$},
]
...
Wie in \cite{ofdm:idft} gezeigt wird, kann das OFDM-Basisbandsignal mit einer \ac{IDFT} und einer anschließenden parallel-seriell Wandlung aus den OFDM Symbolen erzeugt werden. Um noch robuster gegen Mehrwegempfang zu sein, wird jedem OFDM Symbol ein Guard-Intervall der Länge $T_G=246\mu s$ vorangestellt. $T_G$ entspricht damit der Zeit, in der Echos des Vorgängersymbols abklingen können ohne das neue Symbol zu stören. Der Inhalt des Guard-Intervalls entspricht dem Ende des darauf folgenden Symbols. Das hat den Vorteil, dass durch die periodische Wiederholung des Signals kaum Signalanteile wegen Mehrwegeempfang verloren gehen~\cite{nt1}.
